...
class MimeModel(pl.LightningModule):
    def __init__(
        ...
        cls_dict: dict,
        optimizers: list,
        intermediate_cls_hidden_dims: list,
        ...
        if freeze_pretrain:
            logging.info("[Parameters] Freezing multimodal model weights")
            for param in self.multimodal_model.parameters():
                param.requires_grad = False

            logging.info("[Parameters] Freezing reading comprehension model weights")
            for param in self.rc_model.parameters():
                param.requires_grad = False
        ...
        for num_classes in cls_dict.values():
            if len(intermediate_cls_hidden_dims) >= 1:
                seq = nn.Sequential()
                seq.append(nn.Linear(combined_hidden_size, intermediate_cls_hidden_dims[0]))
                
                for idx in range(1, len(intermediate_cls_hidden_dims)):
                    seq.append(nn.Linear(intermediate_cls_hidden_dims[idx - 1], intermediate_cls_hidden_dims[idx]))
                
                seq.append(nn.Linear(intermediate_cls_hidden_dims[-1], num_classes))
                mlps.append(seq)
            else:
                mlps.append(nn.Linear(combined_hidden_size, num_classes))
